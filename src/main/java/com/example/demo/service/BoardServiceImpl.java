...
public class BoardServiceImpl implements BoardService {
	 ...
	 * 
	 * // 데이터베이스에서 게시물 목록가져오기 List<Board> result = repository.findAll();
	 * 
	 * // 엔티티 리스트를 DTO 리스트로 변환하기 List<BoardDTO> list = new ArrayList<>();
	 * 
	 * // 리스트에서 스트림 생성하기 // 스트림을 사용하여 모든 엔티티를 DTO로 변환 // 함수형 인터페이스는 람다식 함수로 구현한다 //
	 * collect: 스트림을 다른 자료구조로 변환하는 함수 list = result.stream() .map(entity ->
